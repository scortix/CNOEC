...
opt = myoptimset;
x_par = myfminunc(funX,initX,opt);
y_par = myfminunc(funY,initY,opt);
z_par = myfminunc(funZ,initZ,opt);

x_m_hat = x_par(4)*sin(2*pi/x_par(3)*t_sim+x_par(2))+x_par(1);
...
xlim([0 t_plot(end)])
legend("Real","Estimated","Init Guess","Position",[0.2 0.6 0 0])
