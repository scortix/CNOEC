                ...
                const emxArray_real_T *fx, emxArray_real_T *gradfx)
{
  ...
  for (m = 0; m < i; m++) {
    if (m + 1 > p->size[1]) {
      emlrtDynamicBoundsCheckR2012b(m + 1, 1, p->size[1], &p_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    loop_ub = p->size[0];
    i1 = b_x->size[0];
    ...
    if (x->size[0] == b_x->size[0]) {
      ...
      d_st.site = &h_emlrtRSI;
      e_binary_expand_op(&d_st, h_emlrtRSI, x, b_x, fun_workspace_tmax,
    ...
    if (m + 1 > J->size[1]) {
      emlrtDynamicBoundsCheckR2012b(m + 1, 1, J->size[1], &o_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    loop_ub = b_x->size[0];
    for (i1 = 0; i1 < loop_ub; i1++) {
