function [X,Y,Z]=moonpos(JD_TT,CooType)
%----------------------------------------------------------------------
% moonpos function       Calculate very accurate ELP2000-82
%                        ecliptic coordinates of the moon
%                        referred to the inertial mean ecliptic
%                        and equinox of date.
...
%                       bug correction 'elpdt'  April 2004
...
JD_TDB = JD_TT + tdb_tdt(JD_TT)./86400.0;
...
con2range = inline('(X./(2.*pi) - floor(X./(2.*pi))).*2.*pi','X');
W1   = con2range(W1);
W2   = con2range(W2);
W3   = con2range(W3);
M    = con2range(M);
