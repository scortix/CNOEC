...
static emlrtRSInfo emlrtRSI = {
    6,      /* lineNo */
    "cost", /* fcnName */
    "P:\\Projects\\CNOEC\\CNOEC_Project\\OrbitPropagator\\V3\\cost.m" /* pathName
                                                                       */
};
...
static emlrtBCInfo emlrtBCI = {
    ...
    "x",    /* aName */
    "cost", /* fName */
    "P:\\Projects\\CNOEC\\CNOEC_Project\\OrbitPropagator\\V3\\cost.m", /* pName
                                                                        */
    0 /* checkKind */
};
          ...
          emxArray_real_T *y, real_T *tCost, real_T *m)
{
  ...
  if (lu + 2.0 > (real_T)x->size[0] - 1.0) {
    ...
    if ((x->size[0] - 1 < 1) || (x->size[0] - 1 > x->size[0])) {
      emlrtDynamicBoundsCheckR2012b(x->size[0] - 1, 1, x->size[0], &e_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    b_x0_tmp = x->size[0] - 1;
  }
  ...
  *tCost = 0.0;
  ...
  for (k = 0; k <= i - 2; k++) {
    ...
    if (k + 1 > qAngle->size[1]) {
      emlrtDynamicBoundsCheckR2012b(k + 1, 1, qAngle->size[1], &f_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    if (k + 1 > csi->size[1]) {
    ...
    st.site = &j_emlrtRSI;
    if (k + 1 > y->size[1]) {
      emlrtDynamicBoundsCheckR2012b(k + 1, 1, y->size[1], &j_emlrtBCI, &st);
    }
    ...
    x0[5] = Ts * (muDoubleScalarSqrt(398600.0 * p) * (lu * lu) +
                  p_mu05 / w *
                      ...
                      Qdiag[2]);
    if (k + 2 > y->size[1]) {
      emlrtDynamicBoundsCheckR2012b(k + 2, 1, y->size[1], &k_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    ...
    *m -= Ts * coeffT * csi_data[k];
    ...
    st.site = &l_emlrtRSI;
    if (k + 1 > t->size[1]) {
      emlrtDynamicBoundsCheckR2012b(k + 1, 1, t->size[1], &m_emlrtBCI, &st);
    }
  ...
  *tCost = *tCost / (real_T)t->size[1] / tmax * 1000.0;
  emxFree_real_T(sp, &t);
  if (y->size[1] < 1) {
    emlrtDynamicBoundsCheckR2012b(y->size[1], 1, y->size[1], &n_emlrtBCI,
                                  (emlrtCTX)sp);
  }
  st.site = &m_emlrtRSI;
  EOE2COE(&st, *(real_T(*)[6]) & topt_data[6 * (y->size[1] - 1)], x0);
  Qdiag[0] = x0[0] - xbar[0];
  Qdiag[1] = x0[1] - xbar[1];
  Qdiag[2] = x0[4] - xbar[4];
  lu = 0.0;
