function J = cost2(x, Ts, tmax, y0, ybar, ratio, Tmax, coeffT, m0, alpha)
...
%     f = f+(xCOE([1:2 5])-xbar([1:2 5]))'*Q*(xCOE([1:2 5])-xbar([1:2 5]))*1.0001^t(k);
end
% f = f/length(t)/tmax*1e3;
f = alpha*tf/100+(1-alpha)*(m0-m)*10;
xCOE = EOE2COE(y(:,end));
xCOEtf = EOE2COE(y(:,indtf));
g = [(xCOE([1:2 5])-xbar([1:2 5]))'*Q*(xCOE([1:2 5])-xbar([1:2 5]));
    (xCOEtf([1:2 5])-xbar([1:2 5]))'*Q*(xCOEtf([1:2 5])-xbar([1:2 5]))];
% g = [];
h = m-500;
