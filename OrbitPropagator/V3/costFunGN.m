function [J,y,tCost,m] = costFunGN(x, Ts, tmax, y0, ybar, ratio, Tmax, coeffT, m0, alpha)
...
for k = 1:length(t)-1
    ...
    F(k) = sqrt(alpha*(xCOE([1:2 5])-xbar([1:2 5]))'*Q*(xCOE([1:2 5])-xbar([1:2 5]))*t(k)/length(t)/tmax); % Time cost update
end
