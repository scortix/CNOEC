function [xstar, fxstar, niter, exitflag, xsequence] = mySQPGrad(fun, x0, options)
...
gxk = zeros(options.nitermax+1, p)';
hxk = zeros(options.nitermax+1, q)'; 
...
%     if niter==5 
%         if strcmp(options.method,'Steepest')
%             options.method = "Gauss-Newton";
%         else
%             options.method = "Steepest";
%         end
%     end
...
%         gradgxkold = gradgxk; gradhxkold = gradhxk;
...
%             theta = 1;
%         else
%             theta = 0.8*(s'*Hk*s)/(s'*Hk*s-s'y);
%         end
%         rk = theta*y+(1-theta)*Hk*s
        ...
        if y'*s <= options.gamma*s'*Hk*s
            y = y + (options.gamma*s'*Hk*s-s'*y)/(s'*Hk*s-s'*y)*(Hk*s-y);
%             if all(~isnan(ynew))
%                 y = ynew;
%             else
