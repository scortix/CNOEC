...
static emlrtRSInfo emlrtRSI = {
    ...
    "Project\\OrbitPropagator\\Unconstrained\\costFun.m" /* pathName */
};
...
static emlrtRSInfo v_emlrtRSI = {
    ...
    "P:\\Projects\\CNOEC\\CNOEC_"
    "Project\\OrbitPropagator\\Unconstrained\\EOEDerivatives.m" /* pathName */
};
...
static emlrtRSInfo kb_emlrtRSI = {
    10,                                                           /* lineNo */
    "exp",                                                        /* fcnName */
    "P:\\Matlab\\R2022a\\toolbox\\eml\\lib\\matlab\\elfun\\exp.m" /* pathName */
};

static emlrtRSInfo lb_emlrtRSI = {
    49,     /* lineNo */
    "mean", /* fcnName */
    "P:\\Matlab\\R2022a\\toolbox\\eml\\lib\\matlab\\datafun\\mean.m" /* pathName
                                                                      */
};
...
static emlrtRSInfo mb_emlrtRSI = {
    ...
    "P:"
    "\\Matlab\\R2022a\\toolbox\\eml\\lib\\matlab\\datafun\\private\\combineVect"
    "orElements.m" /* pathName */
};
...
static emlrtRSInfo nb_emlrtRSI = {
    ...
    "P:"
    "\\Matlab\\R2022a\\toolbox\\eml\\lib\\matlab\\datafun\\private\\blockedSumm"
    "ation.m" /* pathName */
};
...
static emlrtMCInfo emlrtMCI = {
    ...
    "costFun", /* fName */
    "P:\\Projects\\CNOEC\\CNOEC_"
    "Project\\OrbitPropagator\\Unconstrained\\costFun.m" /* pName */
};
                             ...
                             const emxArray_real_T *in5)
{
  ...
  in3_data = in3->data;
  emlrtHeapReferenceStackEnterFcnR2012b((emlrtCTX)sp);
  emxInit_real_T(sp, &r, 1, &y_emlrtRTEI);
  i = in3->size[1];
  stride_0_0 = r->size[0];
  if (in4->size[1] == 1) {
    r->size[0] = i;
  ...
  emxEnsureCapacity_real_T(sp, r, stride_0_0, &y_emlrtRTEI);
  r1 = r->data;
  stride_0_0 = (i != 1);
  ...
  for (i = 0; i < loop_ub; i++) {
    r1[i] = -20.0 * (in3_data[6 * (i * stride_0_0)] / in4_data[i * stride_1_0] -
                     6380.0);
  }
  i = in1->size[0];
  in1->size[0] = ((r->size[0] + in5->size[0]) + in5->size[0]) + 1;
  emxEnsureCapacity_real_T(sp, in1, i, &s_emlrtRTEI);
  in1_data = in1->data;
  in1_data[0] = -20.0 * (in2 - 400.0);
  loop_ub = r->size[0];
  for (i = 0; i < loop_ub; i++) {
    in1_data[i + 1] = r1[i];
  }
  loop_ub = in5->size[0];
  for (i = 0; i < loop_ub; i++) {
    in1_data[(i + r->size[0]) + 1] = -20.0 * (1.0 - in5_data[i]);
  }
  loop_ub = in5->size[0];
  for (i = 0; i < loop_ub; i++) {
    in1_data[((i + r->size[0]) + in5->size[0]) + 1] = -20.0 * in5_data[i];
  }
  emxFree_real_T(sp, &r);
  emlrtHeapReferenceStackLeaveFcnR2012b((emlrtCTX)sp);
}
             ...
             emxArray_real_T *y, real_T *tCost, real_T *m)
{
  ...
  if (muDoubleScalarIsNaN(Ts) || muDoubleScalarIsNaN(tmax)) {
    ...
    b_x0_tmp = t->size[0] * t->size[1];
    t->size[0] = 1;
    nleft = (int32_T)(tmax / Ts);
    t->size[1] = nleft + 1;
    emxEnsureCapacity_real_T(&st, t, b_x0_tmp, &g_emlrtRTEI);
    t_data = t->data;
    for (b_x0_tmp = 0; b_x0_tmp <= nleft; b_x0_tmp++) {
  ...
  *tCost = 0.0;
  ...
  for (k = 0; k <= i - 2; k++) {
    ...
    inb = 6 * k + 1;
    nleft = 6 * k + 2;
    w = (y_data[inb] * b_w_tmp + 1.0) + y_data[nleft] * w_tmp;
    ...
    x0_tmp = 6 * k + 4;
    x0[1] =
        Ts * (p_mu05 * ((Qdiag[0] * w_tmp +
                         ((w + 1.0) * b_w_tmp + y_data[inb]) * Qdiag[1] / w) -
                        (y_data[b_x0_tmp] * w_tmp - y_data[x0_tmp] * b_w_tmp) *
                            y_data[nleft] * Qdiag[2] / w));
    x0[2] =
        Ts * (p_mu05 * ((-Qdiag[0] * b_w_tmp +
                         ((w + 1.0) * w_tmp + y_data[nleft]) * Qdiag[1] / w) +
  ...
  *tCost = *tCost / (real_T)t->size[1] / tmax;
  ...
  if (y->size[1] == b_y->size[1]) {
    ...
    emxEnsureCapacity_real_T(&st, qAngle, i, &s_emlrtRTEI);
    topt_data = qAngle->data;
    ...
    inb = csi->size[0];
    for (i = 0; i < inb; i++) {
      topt_data[i + nx] = -20.0 * (1.0 - csi_data[i]);
    }
    inb = csi->size[0];
    for (i = 0; i < inb; i++) {
      topt_data[(i + nx) + csi->size[0]] = -20.0 * csi_data[i];
    }
  } else {
    b_st.site = &yb_emlrtRSI;
    binary_expand_op(&b_st, qAngle, *m, y, b_y, csi);
    topt_data = qAngle->data;
  }
  emxFree_real_T(&st, &b_y);
  emxFree_real_T(&st, &csi);
  b_st.site = &kb_emlrtRSI;
  nx = qAngle->size[0];
  c_st.site = &ab_emlrtRSI;
  if (qAngle->size[0] > 2147483646) {
    d_st.site = &p_emlrtRSI;
    check_forloop_overflow_error(&d_st);
  }
  for (k = 0; k < nx; k++) {
    topt_data[k] = muDoubleScalarExp(topt_data[k]);
  }
  st.site = &k_emlrtRSI;
  ...
  if (qAngle->size[0] < 4096) {
    f_st.site = &pb_emlrtRSI;
    p = sumColumnB(&f_st, qAngle, qAngle->size[0]);
  } else {
    nx = qAngle->size[0] / 4096;
    inb = nx << 12;
    nleft = qAngle->size[0] - inb;
    p = sumColumnB4(qAngle, 1);
    for (b_x0_tmp = 2; b_x0_tmp <= nx; b_x0_tmp++) {
      p += sumColumnB4(qAngle, ((b_x0_tmp - 1) << 12) + 1);
    }
    if (nleft > 0) {
      f_st.site = &qb_emlrtRSI;
      p += b_sumColumnB(&f_st, qAngle, nleft, inb + 1);
    }
  }
  lu = 0.0;
  for (i = 0; i < 3; i++) {
